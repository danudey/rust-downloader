...
impl CookieManager {
    ...
    /// Fetch cookies for multiple domains using the selected browser strategy
    pub fn fetch_cookies_for_domains(&self, domains: Vec<String>) -> Result<Vec<Cookie>, BrowserError> {
        debug!("Fetching cookies for domains: {:?} using {}", domains, self.browser_name());
        let result = self.strategy.fetch_cookies(domains.clone());
        match &result {
            Ok(cookies) => {
                info!("Successfully fetched {} cookies for domains: {:?}", cookies.len(), domains);
            }
            Err(e) => {
                warn!("Failed to fetch cookies for domains {:?}: {}", domains, e.brief_message());
            }
        }
        result
    }
...
mod tests {
    ...
    #[test]
    fn test_logging_multiple_domains() {
        // Test that multi-domain cookie fetching is logged
        struct MultiDomainTestStrategy;

        impl BrowserStrategy for MultiDomainTestStrategy {
            fn fetch_cookies(&self, domains: Vec<String>) -> Result<Vec<Cookie>, BrowserError> {
                ...
                for domain in domains {
                    cookies.push(Cookie {
                        domain: domain.clone(),
                        path: "/".to_string(),
                        name: format!("cookie_for_{}", domain.replace(".", "_")),
                        value: "test_value".to_string(),
                        http_only: false,
                        secure: false,
                        same_site: 0,
        ...
        let manager = CookieManager::with_strategy(Box::new(MultiDomainTestStrategy));
        let domains = vec!["example.com".to_string(), "test.com".to_string()];
        let result = manager.fetch_cookies_for_domains(domains);
    ...
    #[test]
    fn test_cookie_manager_fetch_cookies_for_domains() {
        // Try to create a manager with auto-detection
        if let Ok(manager) = CookieManager::with_auto_detection() {
            // Test that the method exists and can be called
            let domains = vec!["example.com".to_string(), "test.com".to_string()];
